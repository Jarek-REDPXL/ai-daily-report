...
## Do
1. Read the job spec → note its `target_domains` and `min_count`.
2. Read `reports/data/cards.js`. For EVERY existing card whose `domains` array intersects the
   job's target domains, capture: `id`, `title`, the one-line `summary` gist, and `tags`.
3. Skim adjacent domains the job cross-lists into (e.g. a card may live in two domains) so a
