...
## Do
1. Take the job's candidate angles + the baseline's "white space". Prioritise angles with real,
   checkable evidence over angles you'd have to hand-wave.
2. Research each with live web search/fetch to the job's SOURCE BAR (read it — it overrides any
...
3. For EVERY factual claim, capture the exact source URL it came from. A number with no
...
4. Gather a little more than `min_count` worth of angles so VERIFY can cut weak ones and the
