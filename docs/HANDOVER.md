...
**RedPxl News** is a self-running, self-improving AI + tech + markets intelligence
platform for the RedPxl team — a private knowledge base (not just a news feed) that
researches the AI/tech/finance world every morning, writes a daily briefing (and a
weekly synthesis on Mondays), grades its own past predictions, compounds what it
learns in an in-repo memory, quality-checks itself, and publishes to a live,
password-protected, on-brand site — with zero manual steps day to day.
...
- `reports/data/index.json` — lightweight metadata + a lowercased search blob `q`
  per report. Loaded once by the site for the sidebar/nav/search.
...
- `assets/app.js` — the renderer. Loads index.json, lazy-loads entries, builds the
  ...
  checklist/keyboard-nav/scroll-progress, with legacy fallback to reports.js.
- `assets/styles.css` — the entire RedPxl design system (see §6).
...
- `reports/data/index.json`, `reports/data/entries/*.json` — generated, don't edit.
...
- `docs/knowledge/digest.md` — running distilled judgment: active threads +
  durable lessons + mission + sourcing standard. Read before writing, updated after.
- `docs/knowledge/predictions.md` — every forward-looking call with outcomes logged
  when they resolve (✅ held / ❌ missed / ⚖️ partial). The accountability layer.

### Prompts (the editorial spec the routine follows)
- `docs/prompts/daily.md` — daily briefing instructions: mission, scope
  (AI/agents/skills/Claude-connectors/research/finance+markets/job-market/policy),
  sourcing rules (wide sources incl. YouTube transcripts + social; EVERY source must
  be an exact clickable link), self-learning loop, output format, quality gate step.
...
Its Instructions tell it to: read the knowledge files + recent reports → research
the last ~24h → prepend one dated daily to reports.js (one EVERY run, even quiet
days) → on Mondays also write the weekly + run build_report.py → update the
knowledge files → run check_reports.py (must pass) → commit + push. The
canonical, up-to-date version of these instructions lives in
