...
- `reports/data/index.json` — lightweight metadata (now incl. `domains`) + a
  lowercased search blob `q` per report. A bare array — loaded once by the site for
  the sidebar/nav/search. (Kept array-shaped so app.js needs no change this round.)
- `reports/data/index.meta.json` — sidecar facet `{ domains: [{slug,count}] }` for
  a future sidebar domain filter. Not yet read by app.js (wired in a later UI round).
