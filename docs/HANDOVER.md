...
- `reports/data/index.meta.json` — sidecar facet
  `{ domains: [{slug, count, label, fullLabel}], cards: [{slug, count, label, fullLabel}] }`
  (labels sourced from scripts/domains.js). `domains` powers the sidebar
  domain-filter chips (loaded by app.js with its own try/catch — if missing/empty,
  no chips render and the site behaves exactly as before); `cards` is the per-domain
  card count for the upcoming hubs (ignored by the current app.js).
...
```
{
  id: "card-paid-meta-advantage-plus", // stable slug
  domains: ["paid"],                   // >=1 valid slug from scripts/domains.js
  title, summary, why,                 // what it is / why it matters to us
  how: ["step", ...],                  // how to run it
...
```
The routine CREATEs or UPDATEs a card whenever a deep beat surfaces a durable play
(see `docs/prompts/daily.md` → "Card extraction"): update the canonical card in place
(bump `updated`, raise `confidence`, add the source), set `supersedes`/`status` when a
play replaces an older one.

`scripts/build-data.js` regenerates index.json + entries/ from reports.js AND the
per-domain card files + cards facet from cards.js. It runs **automatically** inside
the quality gate (`scripts/check_reports.py`), which now validates **both**
reports.js and cards.js (unique ids, valid domains, enum-checked
`confidence`/`status`, and `supersedes`/`related` must reference existing cards). The
site falls back to loading reports.js directly if a JSON fetch ever fails (safety net).
...
### Data
- `reports/data/reports.js` — source of truth for the daily/weekly INTAKE feed.
- `reports/data/cards.js` — source of truth for the durable knowledge cards (the
  compounding library; routine writes here too — see the card layer in §3).
- `reports/data/index.json`, `reports/data/entries/*.json`,
  `reports/data/index.meta.json` (domains + cards facet), `reports/data/cards/*.json`
  (per-domain card lists) — all generated, don't edit.
